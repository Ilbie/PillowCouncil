...
Ship Council is a local/self-hosted MVP that runs a 4-round council discussion, then saves the final decision and TODO list.
...
- API keys are stored in OpenCode, not in Ship Council's SQLite DB
- browser login is started by OpenCode and the resulting credential is reused by Ship Council
- Ship Council stores the selected provider, login method, and model in SQLite
- Ship Council reuses the standard OpenCode credential store, so browser logins and saved API keys stay in the same place OpenCode already uses

## Configuration

- no project `.env` file is required
- the app database lives at `data/council.db`
